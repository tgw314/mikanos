...
void DrawEye(uint64_t layer_id_flags, int mouse_x, int mouse_y,
             uint32_t color) {
    const double center_x = mouse_x - kCanvasSize / 2.0 - 4.0;
    const double center_y = mouse_y - kCanvasSize / 2.0 - 24.0;

    const double direction = atan2(center_y, center_x);
    double distance = sqrt(pow(center_x, 2) + pow(center_y, 2));
    distance = std::min<double>(distance, kCanvasSize / 2.0 - kEyeSize / 2.0);
