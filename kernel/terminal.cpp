...
Terminal::Terminal(Task &task, const TerminalDescriptor *term_desc)
    ...
    if (show_window_) {
        window_ = std::make_shared<ToplevelWindow>(
            ...
            screen_config.pixel_format, "MikanTerm");
...
struct ScopeGuard {
    F f;
    explicit ScopeGuard(F f_) : f(std::move(f_)) {}
    ~ScopeGuard() { f(); }
};
}  // namespace
...
void Terminal::ExecuteLine() {
    ...
    char *redir_char = strchr(command, '>');
    ...
    if (redir_char) {
        *redir_char = '\0';
        ...
        for (; isspace(*redir_dest); redir_dest++);
    ...
    if (pipe_char) {
        *pipe_char = '\0';
        ...
        for (; isspace(*subcommand); subcommand++);
        ...
        auto term_desc = new TerminalDescriptor{
            ...
            .show_window = false,
            .files = {pipe_fd, files_[1], files_[2]},
        };
        files_[1] = pipe_fd;
    ...
    if (strcmp(command, "ls") == 0) {
        ...
        if (post_slash) {
            PrintToFD(*files_[2], "%s is not a directory", name);
            exit_code = 1;
            return;
        }
    ...
    if (strcmp(command, "noterm") == 0) {
        auto term_desc = new TerminalDescriptor{
            .command_line = first_arg,
            .exit_after_command = true,
            .show_window = false,
            .files = files_,
        };
    ...
    for (;;) {
        ...
        switch (msg->type) {
            ...
            case Message::kKeyPush:
                if (const auto &arg = msg->arg.keyboard; arg.press) {
                    const auto area = terminal->InputKey(
                        arg.modifier, arg.keycode, arg.ascii);
                    if (show_window) {
                        Message msg =
                            MakeLayerMessage(task_id, terminal->LayerID(),
                                             LayerOperation::DrawArea, area);
...
size_t PipeDescriptor::Read(void *buf, size_t len) {
    ...
    for (;;) {
        ...
        const size_t copy_bytes = std::min<size_t>(arg.len, len);
        memcpy(buf, arg.data, copy_bytes);
        len_ = arg.len - copy_bytes;
        memcpy(data_, &arg.data[copy_bytes], len_);
        return copy_bytes;
    }
}
...
size_t PipeDescriptor::Write(const void *buf, size_t len) {
    ...
    auto &arg = msg.arg.pipe;

    size_t sent_bytes = 0;
    while (sent_bytes < len) {
        arg.len = std::min(len - sent_bytes, sizeof(arg.data));
        memcpy(arg.data, &bufc[sent_bytes], arg.len);
        sent_bytes += arg.len;
        __asm__("cli");
