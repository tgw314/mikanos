...
std::pair<DirectoryEntry *, bool> FindFile(const char *path,
    ...
    while (directory_cluster != kEndOfClusterChain) {
        ...
        for (int i = 0; i < bytes_per_cluster / sizeof(DirectoryEntry); i++) {
            if (dir[i].name[0] == 0x00) goto not_found;
            if (!NameIsEqual(dir[i], path_elem)) continue;
...
bool NameIsEqual(const DirectoryEntry &entry, const char *name) {
    ...
    for (int i = 0, i83 = 0; name[i] != 0 && i83 < sizeof(name83); i++, i83++) {
        if (name[i] == '.') {
            i83 = 7;
            continue;
        }
...
WithError<DirectoryEntry *> CreateFile(const char *path) {
    auto parent_dir_cluster = boot_volume_image->root_cluster;
...
unsigned long AllocateClusterChain(size_t n) {
    ...
    for (first_cluster = 2; fat[first_cluster] != 0; first_cluster++);
