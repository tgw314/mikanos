...
class BitmapMemoryManager {
   public:
    ...
    // ビットマップ配列の 1 つの要素のビット数 == フレーム数
    static const size_t kBitsPerMapLine{8 * sizeof(MapLineType)};

    // インスタンスを初期化する
    BitmapMemoryManager();

    // 要求されたフレーム数の領域を確保して先頭のフレーム ID を返す
     ...
     *
     * range_begin_ メモリ範囲の始点
     * range_end_   メモリ範囲の終点 最終フレームの次のフレーム
     */
