...
void Initialize() {
    ...
    for (int i = 0; i < pci::num_device; i++) {
        if (pci::devices[i].class_code.Match(0x0cu, 0x03u, 0x30u)) {
            ...
            // Intel, AMD 製を優先して xHC を探す
            const bool is_intel = (vendor_id == 0x8086);
            const bool is_amd = (vendor_id == 0x1022);
            if (is_intel || is_amd) break;
        }
    }
    if (!xhc_dev) {
...
void ProcessEvents() {
    while (controller->PrimaryEventRing()->HasFront()) {
        if (auto err = usb::xhci::ProcessEvent(*controller)) {
