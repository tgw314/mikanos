...
global RestoreContext  ; void RestoreContext(void *task_context);
RestoreContext:
    ; iret 用のスタックフレーム
...
global CallApp  ; void CallApp(int argc, char **argv, uint16_t ss, uint64_t rip,
CallApp:        ; uint64_t rsp, uint64_t *os_stack_ptr);
    push rbx
    push rbp
    push r12
    push r13
...
global IntHandlerLAPICTimer  ; void IntHandlerLAPICTimer();
...
global ExitApp  ; void ExitApp(uint64_t rsp, uint32_t ret_val);
ExitApp:
    mov rsp, rdi
    mov eax, esi
