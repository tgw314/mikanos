...
int CountUTF8Size(uint8_t c) {
    if (c < 0x80) return 1;
    if (0xc0 <= c && c < 0xe0) return 2;
    if (0xe0 <= c && c < 0xf0) return 3;
    if (0xf0 <= c && c < 0xf8) return 4;
    return 0;
}
...
void InitializeFont() {
    if (int err = FT_Init_FreeType(&ft_library)) {
        Log(kError, "FT_Init_FreeType: %d", err);
        exit(1);
    }

    auto [entry, pos_slash] = fat::FindFile("/font.ttf");
    if (entry == nullptr || pos_slash) {
        Log(kError, "failed to load font file");
        exit(1);
    }
    ...
    if (fat::LoadFile(nihongo_buf->data(), size, *entry) != size) {
        Log(kError, "failed to read font file completely");
        delete nihongo_buf;
        exit(1);
    }
}
